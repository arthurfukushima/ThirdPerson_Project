...
//		DebugExtension.DebugCircle(position+(_forward*0.5f), direction, color, ((_forward*0.5f)-(slerpedVector.normalized*(dist*0.5f))).magnitude, duration, depthTest);
	...
	/// </param>
	public static void DebugArrow(Vector3 position, Vector3 direction, Color color, float coneRatio = 0.333f, float duration = 0, bool depthTest = true)
	{
		Debug.DrawRay(position, direction, color, duration, depthTest);
		DebugExtension.DebugCone(position+direction, -direction*coneRatio, color, 15, duration, depthTest);
	}
	...
	/// 	- Whether or not the arrow should be faded when behind other objects. 
	/// </param>
	public static void DebugArrow(Vector3 position, Vector3 direction, float coneRatio = 0.333f, float duration = 0, bool depthTest = true)
	{
		DebugArrow(position, direction, Color.white, coneRatio, duration, depthTest);
	}
